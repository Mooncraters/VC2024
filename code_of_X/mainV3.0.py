...
def add_label(my_dict1, y1, y2, mod):
    for i in y1:
        y2.append(my_dict1[i][mod])


add_label(my_dict, y_train, y_color_train, 0)
add_label(my_dict, y_train, y_shape_train, 1)
...
while cnt < 3 and len(X_test_tem) != 0:
    cnt += 1
    clf.fit(X_train, y_train)
    clf_color.fit(X_train, y_color_train)
    clf_shape.fit(X_train, y_shape_train)

    y_predict = clf.predict(X_test_tem)
    y_color_predict = clf_color.predict(X_test_tem)
    y_shape_predict = clf_shape.predict(X_test_tem)

    length = len(X_test_tem)
    X_test_tem2 = []
    y_test_tem2 = []
    for i in range(length):
        if my_dict[y_predict[i]] == (y_color_predict[i], y_shape_predict[i]):
            X_train.append(X_test_tem[i])
            y_train.append(y_test_tem[i])
            y_color_train.append(y_color_predict[i])
            y_shape_train.append(y_shape_predict[i])
        else:
            X_test_tem2.append(X_test_tem[i])
            y_test_tem2.append(y_test_tem[i])
    X_test_tem = X_test_tem2
    y_test_tem = y_test_tem2
